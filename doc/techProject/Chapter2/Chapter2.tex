...
Pearson correlation gives a coefficient value from 1 to -1, 1 showing strong correlation with a positive gradient, and -1 shows strong correlation with a negative change. \textbf{\textit{TODO - find out or lose it}}
...
After the similarities are calculated, the next step is to predict how the user would rate the item in question. A way of doing this is to calculate a weighted sum of the user's previous item ratings. Allowing $ S^{k}(i;u)$ to equal the list of items i user u has rated, the equation below shows how the predicted value is found. \textbf{\textit{TODO - Whats happening}}
...
User-Based Neighbourhood is when you look for users who rate similar to see whether item i is similar to user u. Below shows an equation similar to the one above but instead we look through a list similar users.
...
Item based is when the system predict rating of user u for item i rooted from ratings of u for similar items to i. Similarity between two items are affirmed when multiple users rate them alike. \textbf{\textit{TODO - Make 2 sentences simpler to understand + Highlight difference in item based and user based better}}


\textbf{\textit{TODO - Redo this}}
\subsection{Matrix Factorisation}

\textbf{\textit{TODO - State what the problem is, whats in the matrix and what is a sparse matrix}}

Matrix factorisation is when you take a sparse matrix and instead of storing each rating for each user, you store features that when multiplied together can calculate the rating of an item by a user.

\textbf{\textit{TODO - Explain what features are}}

This is really useful for a sparse matrix, which is usually the case for most recommendation systems, because it shows information that the matrix alone doesnt. Because it factorises the users ratings from features, you can see trends on what groups of users likes and doesnt like. Because it reduces the dimensionality of a given matrix, matrix factorisation requires less processing power than other methods of finding similar items. 

\textbf{\textit{TODO - Explain to 5 year old, use simple language}}
