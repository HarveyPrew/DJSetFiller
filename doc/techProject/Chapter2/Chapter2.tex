...
Item-Based Neighbourhood is when similarites for a given item is found based on the user's previous item ratings. Similarities are found through calculating the similarity and then calculating the prediction. Below shows a matrix with ratings from  $u_{2}, u_{i}$ and $u_{m-1}$. For finding similarities between $i_{i}$ and $i_{k}$, we only take $u_{2}$ and  $u_{i}$ into consideration when using item based neighbourhood because they have rated both item $i_{i}$ and $i_{k}$.  
